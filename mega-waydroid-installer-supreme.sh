#!/bin/bash
# Mega-Script supremo para instalar Waydroid en múltiples distribuciones
# Verificado al 11 de octubre de 2025, 21:00 AST
# Integra: supechicken (Android TV/ChromeOS), n1lby73 (universal), waydroid-package-manager (APKs), Quackdoc (GPU/spoof), WayBak (backups), Droid-NDK-Extractor (ARM), scrcpy (control remoto), DKMS (módulos dinámicos), Distrobox (contenedor moderno), Anbox (alternativa legacy)
# Mejoras: Crosh automatizado, Widevine/VA-API check, Flatpak/Snap, pv, limpieza, README dinámico, soporte musl en Alpine
# Uso: sudo bash mega_waydroid_installer_supreme.sh
# Advertencia: Ejecuta bajo tu propio riesgo; revisa https://docs.waydro.id
...
# Función para instalar binder_linux via DKMS
...
# Detección de distribución ampliada
...
echo "Detalles del equipo al iniciar (11/10/2025 21:00 AST):" | tee -a "$LOG_FILE"
echo "------------------------------------------------" | tee -a "$LOG_FILE"
...
# Ejecutar pre-chequeos
pre_checks

# Menú principal con dialog o select
if command -v dialog &>/dev/null; then
    exec 3>&1
    opt=$(dialog --menu "Selecciona tu distribución o acción:" 18 60 14 \
        1 "Ubuntu/Debian" \
        2 "Arch Linux" \
        ...
        15 "Alpine Linux" \
        16 "Distrobox (Contenedor Moderno)" \
        17 "Anbox (Alternativa Legacy)" \
        18 "Salir" 2>&1 1>&3)
    exec 3>&-
    case $opt in
        1) opt="Ubuntu/Debian" ;;
        ...
        15) opt="Alpine Linux" ;;
        16) opt="Distrobox (Contenedor Moderno)" ;;
        17) opt="Anbox (Alternativa Legacy)" ;;
        18) opt="Salir" ;;
    esac
else
    echo "Bienvenido al mega-instalador supremo de Waydroid." | tee -a "$LOG_FILE"
    options=("Ubuntu/Debian" "Arch Linux" "SteamOS (Steam Deck)" "WSL2 (Windows)" "Install Waydroid Script (casualsnek)" "Setup Full Desktop in WSL2 (tdcosta100)" "openSUSE" "NixOS" "Gentoo" "ChromeOS (Crostini/Flex)" "Universal Fallback (n1lby73)" "Void Linux" "KISS Linux" "Fedora (Silverblue)" "Alpine Linux" "Distrobox (Contenedor Moderno)" "Anbox (Alternativa Legacy)" "Salir")
    select opt in "${options[@]}"; do
        break
    done
fi

# Si se usó sugerencia, overridea la selección
...
function post_install_menu() {
    ...
    case $post_choice in
        1)
            ...
            chmod +x wpm
            echo "Instalado. Uso: ./wpm install <app> o ./wpm apkinstall <path/to/apk>." | tee -a "$LOG_FILE"
            ;;
        2)
...
EOF
    ...
    "Universal Fallback (n1lby73)")
        ...
        echo "Créditos: n1lby73 (https://github.com/n1lby73/waydroid-installer)"
        git clone https://github.com/n1lby73/waydroid-installer || handle_error "Falló el clon de repositorio."
        cd waydroid-installer || handle_error "Falló el cambio de directorio."
        chmod +x requirements.sh
        ./requirements.sh || handle_error "Falló la ejecución del instalador universal."
        echo "¿Usar modo Android TV de supechicken? (s/n, predeterminado Android 11 vanilla)" | tee -a "$LOG_FILE"
        read tv_mode
        if [[ "$tv_mode" == "s" || "$tv_mode" == "S" ]]; then
            echo "Descargando build Android TV..." | tee -a "$LOG_FILE"
    ...
    "Alpine Linux")
        ...
        sudo apk add --no-cache community || echo "Repositorio community ya habilitado." | tee -a "$LOG_FILE"
    ...
    "Distrobox (Contenedor Moderno)")
        echo "Instalando Waydroid via Distrobox (OCI + Podman)..." | tee -a "$LOG_FILE"
        echo "Créditos: ublue-os/waydroid (https://github.com/ublue-os/waydroid)" | tee -a "$LOG_FILE"
        
        # Verificar Podman
        if ! command -v podman &>/dev/null; then
            echo "Instalando Podman..." | tee -a "$LOG_FILE"
            if command -v apt &>/dev/null; then
                sudo apt update && sudo apt install -y podman || handle_error "Falló instalación de Podman."
            elif command -v pacman &>/dev/null; then
                sudo pacman -S --needed podman || handle_error "Falló instalación de Podman."
            elif command -v dnf &>/dev/null; then
                sudo dnf install -y podman || handle_error "Falló instalación de Podman."
            elif command -v apk &>/dev/null; then
                sudo apk add podman || handle_error "Falló instalación de Podman."
            else
                handle_error "Podman no disponible en este sistema."
            fi
        fi
        
        # Verificar Distrobox
        if ! command -v distrobox &>/dev/null; then
            echo "Instalando Distrobox..." | tee -a "$LOG_FILE"
            curl -s https://raw.githubusercontent.com/89luca89/distrobox/main/install | sudo sh || handle_error "Falló instalación de Distrobox."
        fi
        
        # Crear contenedor Waydroid
        echo "Creando contenedor Waydroid..." | tee -a "$LOG_FILE"
        distrobox create --root --name waydroid-box --image ghcr.io/ublue-os/waydroid:latest --additional-packages "weston pulseaudio" --gpu --pre-install-hook "sudo waydroid init" || handle_error "Falló creación del contenedor."
        
        # Exportar apps al host
        echo "Exportando aplicaciones al host..." | tee -a "$LOG_FILE"
        distrobox-export --app waydroid-session --root waydroid-box || echo "Advertencia: Exportación falló, usa 'distrobox enter waydroid-box' manualmente." | tee -a "$LOG_FILE"
        distrobox-export --app waydroid-ui --root waydroid-box || echo "Advertencia: UI exportación falló." | tee -a "$LOG_FILE"
        
        # Configurar binds para binder/ashmem
        echo "Configurando dispositivos Android..." | tee -a "$LOG_FILE"
        sudo mkdir -p /var/lib/waydroid
        sudo mount --bind /dev/binder /var/lib/waydroid/binder || echo "Advertencia: /dev/binder no disponible." | tee -a "$LOG_FILE"
        sudo mount --bind /dev/ashmem /var/lib/waydroid/ashmem || echo "Advertencia: /dev/ashmem no disponible." | tee -a "$LOG_FILE"
        
        # Opcional: Android TV mode
        echo "¿Usar modo Android TV? (s/n)" | tee -a "$LOG_FILE"
        read tv_mode
        if [[ "$tv_mode" == "s" || "$tv_mode" == "S" ]]; then
            distrobox enter --root waydroid-box -- sudo waydroid init -s TV || echo "Advertencia: Modo TV no disponible en imagen base." | tee -a "$LOG_FILE"
        fi
        
        echo "¡Waydroid en Distrobox listo! Ejecuta 'waydroid-session' desde el menú o 'distrobox enter waydroid-box'." | tee -a "$LOG_FILE"
        echo "Para gestión: podman ps, distrobox list, distrobox stop waydroid-box" | tee -a "$LOG_FILE"
        post_install_menu
        ;;

    "Anbox (Alternativa Legacy)")
        echo "Instalando Anbox (alternativa legacy a Waydroid)..." | tee -a "$LOG_FILE"
        echo "Advertencia: Anbox es legacy y depreciado. Usa Waydroid para mejor soporte." | tee -a "$LOG_FILE"
        echo "Créditos: Anbox Team (https://anbox.io)" | tee -a "$LOG_FILE"
        case "$DISTRO" in
            ubuntu|debian)
                sudo apt update && sudo apt install -y linux-modules-extra-$(uname -r) anbox anbox-modules-dkms || handle_error "Falló instalación de Anbox."
                ;;
            arch|endeavour|manjaro)
                sudo pacman -S --needed anbox-git anbox-modules-arch-dkms-git || handle_error "Falló instalación de Anbox."
                ;;
            fedora)
                sudo dnf install -y anbox anbox-modules-dkms || handle_error "Falló instalación de Anbox."
                ;;
            opensuse*)
                sudo zypper install -y anbox anbox-kmp || handle_error "Falló instalación de Anbox."
                ;;
            alpine)
                sudo apk add anbox anbox-modules-dkms || handle_error "Falló instalación de Anbox."
                ;;
            *)
                handle_error "Anbox no soportado en esta distro. Usa el fallback universal."
                ;;
        esac
        sudo modprobe ashmem_linux binder_linux || handle_error "Falló el carga de módulos Anbox."
        if command -v systemctl &>/dev/null; then
            sudo systemctl enable anbox-container-manager || handle_error "Falló la habilitación del servicio Anbox."
            sudo systemctl start anbox-container-manager || handle_error "Falló el inicio del contenedor Anbox."
        fi
        echo "Instalación completada. Ejecuta 'anbox launch --package=org.anbox.appmgr --component=org.anbox.appmgr.AppViewActivity' para iniciar." | tee -a "$LOG_FILE"
        post_install_menu
        ;;
    ...
    "Salir")
        ...
        exit 0
        ;;
    *) echo "Opción inválida. Elige del 1 al 18." ;;
esac
...
cat << EOF > "$README_FILE"
# Waydroid Setup - Resumen de Instalación
*Fecha de instalación*: $(date)
*Distribución*: $DISTRO ($SUGGESTED_OPT)
*libc*: $(if ldd --version 2>&1 | grep -q musl; then echo "musl (gcompat instalado)"; else echo "glibc u otra"; fi)
